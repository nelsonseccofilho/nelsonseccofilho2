...
describe('CaseNavigation', () => {
  it.each([
  ...
  ] as const)('continues from %s to the next featured case', (projectId, nextProjectName, nextHref) => {
    ...
    expect(within(navigation).getByRole('link', { name: new RegExp(`Next case.*${nextProjectName}`, 'i') })).toHaveAttribute('href', nextHref);
  });

  it('preserves Portuguese routes and returns directly to the cases anchor', () => {
    ...
    expect(within(navigation).getByRole('link', { name: /Próximo case.*SUBITER/i })).toHaveAttribute('href', '/projetos/subiter');
    expect(Array.from(navigation.querySelectorAll('a')).every((link) => !link.getAttribute('href')?.startsWith('/en'))).toBe(true);
  });

  it('does not invent a next case after DASA', () => {
