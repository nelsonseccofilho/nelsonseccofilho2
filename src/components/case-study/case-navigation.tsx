...
import { getHomeCasesPath, getLocalizedPath } from '@/i18n/routes';
    ...
    <nav id="case-navigation" className="py-[clamp(2rem,4vw,4rem)]" aria-label={common.accessibility.caseNavigation}>
      <div className="layout-container grid gap-3 sm:grid-cols-2 sm:items-stretch">
        <Link
          ...
          className="inline-flex min-h-11 items-center justify-center rounded-[var(--radius-pill)] border border-[var(--color-border)] bg-[var(--color-surface-elevated)] px-6 text-center font-semibold text-[var(--color-text-primary)] no-underline transition-colors hover:border-[var(--color-brand)] hover:bg-[var(--color-surface)] hover:text-[var(--color-brand-text)] motion-reduce:transition-none sm:justify-self-start"
        >
        ...
        {nextProjectId ? (
          ...
          >
            <span className="font-semibold">{common.caseNavigation.nextCaseLabel} →</span>
