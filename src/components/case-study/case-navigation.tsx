...
import { getCaseNavigationProjectOpenEvent } from '@/components/analytics/clarity';
...
export function CaseNavigation({ locale, projectId }: CaseNavigationProps) {
  ...
  const nextCaseProjectId = nextProjectId && nextProjectId !== 'horizon-his' ? nextProjectId : null;
      ...
      <div className="layout-container grid gap-3 sm:grid-cols-2 sm:items-stretch">
        ...
        {nextCaseProjectId ? (
          <AnalyticsNavigationLink
            href={getLocalizedPath(nextCaseProjectId, locale)}
            eventName={getCaseNavigationProjectOpenEvent(nextCaseProjectId)}
          ...
          >
            ...
            <span className="text-xs text-[var(--color-text-secondary)]">{projectFacts[nextCaseProjectId].projectName}</span>
          </AnalyticsNavigationLink>
        ) : null}
      </div>
    </nav>
